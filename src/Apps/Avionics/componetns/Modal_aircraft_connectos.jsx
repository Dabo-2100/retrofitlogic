...
export default function Modal_aircraft_connectos() {
    ...
    const handleSearch = () => {
        ...
        let final = searchRes.filter((el) => { return el.connector_name.toLowerCase().includes(searchVal) })
        setViewRows(final);
    }
    ...
    const handlePrint = () => {
        ...
        `);
        newWin.document.write('</head><body class="p-5" onload="window.print();window.close()">');
    ...
    return (
        <div className="Modal" onClick={handleCloseModal}>
            <div className="content animate__animated animate__fadeInDown p-3 d-flex flex-wrap" onClick={(e) => { e.stopPropagation() }}>
                <h1 className="col-12 header">View aircraft S/N : {modalData.addConnectorsToSN} Connectors</h1>
                <div className="col-12">
                    <div className="col-12">
                        <input onKeyUp={handleSearch} className="col-12 form-control" type="search" placeholder="Search Connectors by S/N" />
                    </div>
                    <div className="col-12 d-flex my-3 align-items-center justify-content-between">
                        <p className="m0">Advanced Search</p>
                        <div className="d-flex gap-3 align-items-center">
                            <button className="btn btn-success" onClick={getAircraftConnectors}>Show All</button>
                            <FontAwesomeIcon onClick={handlePrint} style={{ fontSize: "1.5rem" }} icon={faPrint} />
                        </div>
                    </div>
                    ...
                    <table className="table table-bordered">
                        <tbody>
                            <tr>
                                ...
                                <td>
                                    <div className="d-flex align-items-center gap-1 col-12">
                                        <input id="s1Open" type="checkbox" onChange={(e) => { e.target.checked ? setS1Open(1) : setS1Open('not') }} />
                                        <label htmlFor="s1Open">Open</label>
                                    </div>
                                </td>
                                <td>
                                    <div className="d-flex align-items-center gap-1 col-12">
                                        <input id="s1Close" type="checkbox" onChange={(e) => { e.target.checked ? setS1Close(0) : setS1Close('not') }} />
                                        <label htmlFor="s1Close">Closed</label>
                                    </div>
                                </td>
                                <td>
                                    <div className="d-flex align-items-center gap-1 col-12">
                                        <input id="s1Removed" type="checkbox" onChange={(e) => { e.target.checked ? setS1Removed(2) : setS1Removed('not') }} />
                            ...
                            <tr>
                                ...
                                <td>
                                    <div className="d-flex align-items-center gap-1 col-12">
                                        <input id="s2Open" type="checkbox" onChange={(e) => { e.target.checked ? setS2Open(1) : setS2Open('not') }} />
                                        <label htmlFor="s2Open">Orignial</label>
                                    </div>
                                </td>
                                <td>
                                    <div className="d-flex align-items-center gap-1 col-12">
                                        <input id="s2Close" type="checkbox" onChange={(e) => { e.target.checked ? setS2Colse(0) : setS2Colse('not') }} />
                                        <label htmlFor="s2Close">Installed</label>
                                    </div>
                                </td>
                                <td>
                                    <div className="d-flex align-items-center gap-1 col-12">
                                        <input id="s2Removed" type="checkbox" onChange={(e) => { e.target.checked ? setS2Removed('null') : setS2Removed('not') }} />
                ...
                </div>
                {
                    viewRows.length > 0 ? (
                                ...
                                {viewRows.map((connector, index) => {
                                    return (
                                        <tr key={index}>
                                            ...
                                            <td>
                                                <select value={connector.is_open} onChange={() => changeOpenStatus(connector.log_id, "is_open")} className={`form-select ${connector.is_open == 0 ? "redStatus" :
                                                    connector.is_open == 1 ? "greenStatus" :
                                                        connector.is_open == 2 ? "yellowStatus" : "bg-info"
                                                    }`}>
                                            ...
                                            <td>
                                                <select value={connector.is_orignal} onChange={() => changeOpenStatus(connector.log_id, "is_orignal")} className={`form-select ${connector.is_orignal == 0 ? "greenStatus" :
                        ...
                        <div className="col-12 d-flex flex-wrap">
                            <p className="alert alert-danger col-12">There is no connectors with that S/N Added to Aircraft </p>
                            <button onClick={() => setActiveModal(7)} className="btn btn-success">Add it</button>
                        </div>
                }
