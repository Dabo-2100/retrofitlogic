...
export default function ItemsFinder() {
  ...
  function getSearchResults() {
    axios
      .post(
        `${Server_Url}/php/index.php/api/warehouse/products`,
        {
  ...
  return (
    <div className="ItemsFinder col-12">
      <form
        className="col-12 searchBar d-flex align-items-center p-3 gap-3"
        onSubmit={handelSearch}
      >
        <FontAwesomeIcon icon={faMagnifyingGlass} />
        <input
          className="form-control"
          placeholder="Find Items By P/N , Name or USA P/N"
          onKeyUp={handelSearchValue}
        />
      ...
      <div className="col-12 searchResults px-3">
        {searchResult.length > 0 ? (
          <React.Fragment>
            <table
              className="col-12 table table-dark table-bordered table-hover"
              ...
              {pages.map((p, index) => {
                return (
                  <button
                    ...
                    className={`btn ${
                      index == activePage ? "btn-info" : "btn-primary"
                    }`}
                  >
                    {p}
                  </button>
