...
export default function Retrofit_Data() {
    ...
    const [tabs] = useState(["Aricrafts", "Service Bullitens", "SB Parts", "SB Tasks", "Departements"]);
    return (
        <div
            id="Retrofit_Data"
            ...
            <ul className="nav nav-tabs col-12" id="retrofitTabs">
                {
                    tabs.map((tab, index) => {
                        return (
                            <li className="nav-item" key={index} onClick={() => setActiveTab(index)}>
                                <a className={`nav-link ${activeTab == index ? 'active' : null}`} aria-current="page" href="#">{tab}</a>
                            </li>
                        )
                    })
                }
