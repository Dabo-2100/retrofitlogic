...
export const useMakeDate = (date) => {
    ...
    useEffect(() => {
        ...
        let final = `${now.getFullYear()}-${(now.getMonth() + 1) < 10 ? `0${now.getMonth() + 1}` : (now.getMonth() + 1)}-${now.getDate() < 10 ? `0${now.getDate()}` : now.getDate()}`
        setRes(final);
    }, []);
    return res;
};
...
export const useProjectStatus = () => {
    ...
    const [res, setRes] = useState([]);
    useEffect(() => {
        axios.get(`${Server_Url}/php/index.php/api/projects/status`, {
            headers: {
...
export const useAllTeams = () => {
    ...
    const [res, setRes] = useState([]);
    useEffect(() => {
        axios.get(`${Server_Url}/php/index.php/api/projects/teams`, {
            headers: {
                Authorization: `Bearer ${token}`,
            }
        }).then((res) => {
...
export const useProjectTaskLists = (project_id) => {
    ...
    const [token] = useRecoilState($Token);
    useEffect(() => {
        axios.get(`${Server_Url}/php/index.php/api/projects/${project_id}/tasklists`, {
            headers: {
                Authorization: `Bearer ${token}`,
            }
        }).then((res) => {
...
export const getDueDate = (startDatetime, durationHours) => {
    ...
    let now = dueDate;
    let final = `${now.getFullYear()}-${(now.getMonth() + 1) < 10 ? `0${now.getMonth() + 1}` : (now.getMonth() + 1)}-${now.getDate() < 10 ? `0${now.getDate()}` : now.getDate()}T${now.getHours() < 10 ? `0${now.getHours()}` : now.getHours()}:${now.getMinutes() < 10 ? `0${now.getMinutes()}` : now.getMinutes()}`
    return final;
}