...
export default function NewProject() {
  ...
  const aircraftFleet = useAircraftFleet();
  const today = getTodayDate();
  const projectName = useRef();
  const aircraftSelect = useRef();
  ...
  const handleSubmit = () => {
    ...
    let project_obj = {
      ...
      Values: [
        ...
        aircraftSelect.current.value,
      ],
    };

    axios
      .post(`${Server_Url}/php/index.php/api/insert`, project_obj, {
        headers: {
  ...
  return (
    <div className="Modal" id="newProjectModal" onClick={() => closeModal()}>
      <div
        className="content p-3 d-flex flex-wrap animate__animated animate__fadeInDown"
        ...
        <form className="col-12 d-flex flex-wrap gap-3" onSubmit={handleSubmit}>
          ...
          <div className="col-12 d-flex flex-wrap gap-1">
            ...
            >
              ...
              {aircraftFleet.map((el, index) => {
                return (
                  <option key={index} value={el.aircraft_id}>
                    {el.aircraft_serial_no}
                  </option>
                );
              })}
            </select>
