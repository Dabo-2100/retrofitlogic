...
export function Api() {
    ...
    const user_login = (user_email, user_password) => {
        axios.post(`${Server_Url}/php/index.php/api/auth/login`, {
            ...
            if (res.err == false) {
                ...
                remember_me == true ? localStorage.setItem("user_info", JSON.stringify(res.data)) : sessionStorage.setItem("user_info", JSON.stringify(res.data));
                Swal.fire({
                    icon: "success",
    ...
    const get_data_from_zoho = () => {
        axios.post("https://www.zohoapis.com/crm/v2/functions/get_data/actions/execute?auth_type=apikey&zapikey=1003.6f48757796b7b7657ba3026676537e65.ef98a62d5eba8fd19d777611abef025b")
            .then((res) => {
            })
            .catch((err) => { })
    }
    ...
    const insert_data = async (table_name, Fields, Values) => {
        await axios.post(`${Server_Url}/php/index.php/api/insert`, {
    ...
    const get_data = () => {
        axios.post(`${Server_Url}/php/zoho_functions.php`, {
            api_name: "getData",
        }).then((res) => {
            let total = res.data;
            let start = 1001;
            ...
            let Fields = ["department_id", "department_name", "task_id", "task_name"];
