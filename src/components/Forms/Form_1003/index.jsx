...
export default function Form_1003() {
    const [data] = useState([1, 2, 3, 4, 5, 6]);
    const [, SetResizer] = useRecoilState($Resizer);
    const [formData] = useRecoilState($FormData); const makeSheetNo = (no) => {
    ...
    return (
        <div id="Form_1003" className="animate__animated animate__fadeIn" onContextMenu={openResizer} onClick={(e) => { e.stopPropagation(); closeResizer() }}>
                ...
                <table className="table table-bordered col-12 mb-0">
                    <thead>
                        <tr>
                            ...
                            <th rowSpan={2} style={{ fontSize: "9px" }}>Operator<br />Code</th>
                            ...
                            data.map((row, index) => {
                                return (
                                    <React.Fragment key={row}>
                                        <tr>
                                            <td onContextMenu={toggleMark} rowSpan={2} style={{ width: "3%" }}>{index + 1}</td>
                                            <td rowSpan={2} style={{ width: "27%" }}><textarea className="col-12" style={{ fontSize: "18px" }}></textarea></td>
                                            <td style={{ width: "5%" }}><input style={{ fontSize: "18px" }} className="col-12" /></td>
                                            <td rowSpan={2} style={{ width: "27%" }}><textarea className="col-12" style={{ fontSize: "12px" }} ></textarea></td>
                                            <td rowSpan={2} style={{ width: "5%", position: "relative" }}>
                                                <span className="cross"></span>
                                                <textarea className="col-12"></textarea>
                                            </td>
                                            <td style={{ width: "5%" }}><input className="col-12" /></td>
                                            <td style={{ width: "9%" }}><input className="col-12" /></td>
                                            <td rowSpan={2} style={{ width: "5%" }}><textarea className="col-12"></textarea></td>
                                            <td style={{ width: "5%" }}><input className="col-12" /></td>
                                            <td style={{ width: "9%" }}><input className="col-12" /></td>
                                        </tr>
                                        <tr>
                                            <td><input className="col-12" /></td>
                                            <td><input className="col-12" /></td>
                                            <td><input className="col-12" /></td>
                                            <td><input className="col-12" /></td>
                                            <td><input className="col-12" /></td>
            ...
            <div className="formFooter col-12 d-flex justify-content-end" style={{ paddingRight: "2rem" }}>
                <label className="py-2" style={{ color: "red" }}>IPACO Form 1003</label>
            </div>
        </div >
    )
}
